...
#define SEND_HOUR 15 // 4 PM
#define SEND_MINUTE 39 // 30 minutes past the hour
// Set your timezone here, e.g., "GMT-5" for EST
...
bool is_time_to_send(time_t now) {
    // Calculate today's scheduled send time
    struct tm timeinfo;
    localtime_r(&now, &timeinfo);
    timeinfo.tm_hour = SEND_HOUR;
    timeinfo.tm_min = SEND_MINUTE;
    timeinfo.tm_sec = 0;
    time_t todays_schedule = mktime(&timeinfo);

    // Condition: Current time is past today's schedule, AND the last send was before today's schedule.
    bool should_send = (now >= todays_schedule) && (last_send_time < todays_schedule);

    ESP_LOGI("is_time_to_send", "now: %lld, last_send: %lld, todays_schedule: %lld, should_send: %d", (long long)now, (long long)last_send_time, (long long)todays_schedule, should_send);
...
static void scheduled_sender_task(void*) {
    ESP_LOGE("SENDER_TASK", "!!!!!!!!!! TASK IS RUNNING !!!!!!!!!!");
    while(1) {
        time_t now;
        time(&now);
        ...
        if (is_time_to_send(now)) {
            if (vehicle_count > 0) {
                ESP_LOGI(TAG, "Scheduled interval elapsed. Sending data...");
                ...
                ESP_LOGI(TAG, "Scheduled interval elapsed. No vehicles detected, not sending data.");
                last_send_time = now;
                write_time_to_nvs("last_send_time", last_send_time);
            }
        }
